...
# Load the dataset
df = pd.read_csv('C:\Users\hp\Desktop\py', encoding='latin1')
...
if 'Votes Polled' in df.columns and 'Electors' in df.columns:
    ...
    plt.figure(figsize=(10, 6))
    sns.barplot(data=top_10_turnout, x='Voter Turnout', y='Constituency', hue='State/UT')
...
if 'State/UT' in df.columns and 'Margin' in df.columns:
    state_margin = df.groupby('State/UT')['Margin'].mean().sort_values(ascending=False)
